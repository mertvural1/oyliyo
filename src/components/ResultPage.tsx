...
import { Aurora, Eyebrow, Logo } from './Brand'
...
export const ResultPage = ({ poll, roomCode }: ResultPageProps) => {
  ...
  return <main className="relative min-h-svh overflow-hidden bg-[#fafaff] pb-8 text-[#101132]"><Aurora />
    <header className="relative z-10 mx-auto flex max-w-7xl items-center justify-between px-6 py-6 sm:px-10"><Logo /></header>
    <section className="relative z-10 mx-auto mt-6 w-[calc(100%-2rem)] max-w-265 sm:mt-11 sm:w-[calc(100%-3rem)]">
      <div className="mx-auto max-w-4xl text-center"><h1 className="mt-3 text-4xl font-extrabold leading-tight tracking-[-.06em] sm:text-6xl">{livePoll?.question}</h1><p className="mt-2 text-sm text-[#777595]">{selectedChoiceId ? 'Seçimin kaydedildi. Dilediğin zaman değiştirebilirsin.' : 'Lütfen bir seçenek seç.'}</p></div>
