...
export const QuestionScreen = ({ question, onQuestionChange, onNext }: QuestionScreenProps) => {
  ...
  return <div className="w-full max-w-4xl animate-in fade-in slide-in-from-bottom-3 duration-500 text-center">
    <h1 className="text-[clamp(36px,6vw,52px)] font-extrabold leading-none tracking-[-.06em]">Sorunu yaz.<br />Gerisini <em className="not-italic text-[#5b2cff]">Oyliyo.</em></h1>
    <p className="text-lg text-[#777595] mt-3">Hızlı bir şekilde anket oluştur ve anlık sonucu gör.</p>
    <div className="mt-10 flex items-center gap-3">
      <div className="flex-1 overflow-hidden rounded-2xl border-2 border-[#7548ff] bg-white text-left shadow-[0_16px_32px_rgba(84,44,255,.13)] transition focus-within:ring-4 focus-within:ring-[#dfd5ff]">
        <textarea autoFocus className="h-40 w-full resize-none bg-transparent px-5 pt-5 text-2xl font-medium text-[#101132] outline-none placeholder:font-normal placeholder:text-[#9d9ab7]" value={question} onChange={event => onQuestionChange(event.target.value)} onKeyDown={handleKeyDown} maxLength={200} placeholder={'Örneğin...\nBu akşam ne yiyelim?'} />
